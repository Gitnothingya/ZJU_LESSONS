...
public:
   ...
   */
  static constexpr size_t GetMaxSupportedSize() { return 8 * MAX_CHARS; } //一个bit对应一个数据页的分配情况，1 Byte = 8 bits
...
private:
  ...
  [[maybe_unused]] uint32_t page_allocated_ = 0;        //已经分配的页数
  [[maybe_unused]] uint32_t next_free_page_ = 0;		//下一个空闲数据页的bit_index
  [[maybe_unused]] unsigned char bytes[MAX_CHARS] = {0};  //存Bitmap Content
};
