...
private:
  ...
  char data_[PAGE_SIZE]{};  //存数据的内存空间
  /** The ID of this page. */
  page_id_t page_id_ = INVALID_PAGE_ID; //物理页id,即磁盘管理中的逻辑页id
  /** The pin count of this page. */
  int pin_count_ = 0;     //固定该页面的线程数
  /** True if the page is dirty, i.e. it is different from its corresponding page on disk. */
  bool is_dirty_ = false; //是否为脏页，标识被替换前是否需要将内容存储到磁盘中
  /** Page latch. */
