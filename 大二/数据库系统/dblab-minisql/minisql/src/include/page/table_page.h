...
private:
  ...
  /**
   * @return 当前page中剩余空间的字节size
   */
  uint32_t GetFreeSpaceRemaining() {
    return GetFreeSpacePointer() - SIZE_TABLE_PAGE_HEADER - SIZE_TUPLE * GetTupleCount();
  }

  uint32_t GetTupleCount() { return *reinterpret_cast<uint32_t *>(GetData() + OFFSET_TUPLE_COUNT); }  //为什么不设成公有？

  /**
   * @brief 获得此tuple的offset的值
   * 
   * @param slot_num 
   * @return uint32_t 
   */
...
private:
  ...
  //存每个tuple的offset和size需要的字节数(4 + 4)
