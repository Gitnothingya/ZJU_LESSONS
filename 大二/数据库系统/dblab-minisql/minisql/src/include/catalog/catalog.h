...
/**
 * @brief 位于index和table的上层，用于记录和管理表和索引的元信息页。自身默认被序列化至逻辑页0
 * 
 */
class CatalogMeta {
...
private:
  ...
  //添加构造函数
  explicit CatalogMeta(const std::map<table_id_t, page_id_t> &table_meta_pages, const std::map<index_id_t, page_id_t>& index_meta_pages);

private:
  static constexpr uint32_t CATALOG_METADATA_MAGIC_NUM = 89849;
  std::map<table_id_t, page_id_t> table_meta_pages_;  //记录表的元信息页
  std::map<index_id_t, page_id_t> index_meta_pages_;  //记录索引的元信息页
};
...
public:
  ...
  /**
   * @brief index_key表示创建索引时，哪几列需要作为索引键(用列名表示)
   */
...
private:
  ...
  // memory heap
  MemHeap *heap_; //只用于创建对象，TableInfo自己的成员以及反序列化元数据时的空间由其自己的heap来管理
};
