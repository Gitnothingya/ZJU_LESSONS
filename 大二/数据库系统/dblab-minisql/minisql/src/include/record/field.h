...
protected:
  union Val {
    ...
    float float_;
    char *chars_;
  } value_;                     //数据，联合，共用空间，大小为8字节
  TypeId type_id_;              //数据类型
  uint32_t len_;                //数据字节长度，int和float都为4，char根据实际长度来
  bool is_null_{false};         //是否为空值
  bool manage_data_{false};     //是否需要为数据new空间，此空间是否是new出来的，仅针对字符串内容而言
};
