...
- Equivalent Expressions 等价的关系代数表达式
  ...
  - Cost-based optimization基于cost的优化
    ...
    - cost的估算
      ...
      - 算法的消耗
  - **等价表达式的规则**
- 合取选择和选两次等价：$\sigma_{\theta_1\and\theta_2}(E)=\sigma_{\theta_1}(\sigma_{\theta_2}(E))$ *即同时满足两个条件和一个个去满足等价
  ...
  - $\sigma_{\theta}(E_1\times E_2)=E_1\Join_{\theta}E_2$  *是个定义
    - $\sigma_{\theta_1}(E_1\Join_{\theta_2}E_2)=E_1\Join_{\theta_1\and\theta_2}E_2$ *和上面同理
    - Theta-join和自然连接可以改变连接的两张表的顺序：$E_1\Join_{\theta}E_2=E_2\Join_{\theta}E_1$ *不影响笛卡尔积的结果
    - 自然连接满足结合律：$(E_1\Join E_2)\Join E_3=E_1\Join (E_2\Join E_3)$ *在共有属性上值相等
    - Theta-join的结合规则
  - $(E_1\Join_{\theta_1} E_2)\Join_{\theta_2\and\theta_3} E_3=E_1\Join_{\theta_1\and\theta_3} (E_2\Join_{\theta_2} E_3)$  *$\theta_2$中的属性仅来自$E_2$和$E_3$
    - 选择操作的优化
      - 当$\theta_1$ 中的属性都**只出现在E1中**的时候：$\sigma_{\theta_1}(E_1\Join_{\theta_2}E_2)=\sigma_{\theta_1}(E_1)\Join_{\theta_2}E_2$ 
        - *减小笛卡尔积的规模，从而降低在笛卡尔积上的选择操作的cost
