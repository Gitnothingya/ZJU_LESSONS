...
test_csrs: 	slli s9, s9, 1  # test JUMP and BRANCH
    ...
    addi x0, x0, 0 #暂时替代 #csrrsi  s0, mstatus, 8  #44 300
    addi    a0, zero, 8     # a0 = 8
    slli    a1, a0, 4       # a1 = 0x80
    or      a0, a0, a1      # a0 = 0x88
    slli    a1, a1, 4       # a1 = 0x800
    or      a0, a0, a1      # a0 = 0x888
    addi x0, x0, 0 #暂时替代 #csrrw   zero, mie, a0   #50 304 mie = 0x888 PC = 0xc4 50
...
test_illegal: slli s9, s9, 1  
    sw s9, 0(s11) #第二盏灯亮
	addi x0, x0, 0 #暂时替代非法指令 0xFFFFFFFF
test_load_misalign: slli s9, s9, 1  
    ...
    sw		a1, 0(a0)  #sw不对齐
    addi	a0, zero, 0
    addi x0, x0, 0 #暂时替代 beq x0, x0, 2 0x00000167
loop:   addi    s0, zero, 15
    addi    s0, s0, 15
    jal     zero, loop
trap_m: sw		s10, 0(s11) #进中断，全亮
    addi x0, x0, 0 #暂时替代 #csrrs   a0, mcause, zero # 如果是ecall导致的，则a0 = 0xb 342
    blt     a0, x0, no_add_4
    addi x0, x0, 0 #暂时替代 #csrrs   t0, mepc, zero
    addi    t0, t0, 4
    addi x0, x0, 0 #暂时替代 #csrrw   zero, mepc, t0
no_add_4:    addi x0, x0, 0 #暂时替代 #mret